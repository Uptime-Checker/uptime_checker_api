defmodule UptimeCheckerWeb.Api.V1.RegionController do
  ...
  def create(conn, %{"region" => region_params}) do
    with {:ok, %Region{} = region} <- Region_S.create_region(region_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "region" => region_params}) do
    ...
    with {:ok, %Region{} = region} <- Region_S.update_region(region, region_params) do
      render(conn, "show.json", region: region)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    ...
    with {:ok, %Region{}} <- Region_S.delete_region(region) do
