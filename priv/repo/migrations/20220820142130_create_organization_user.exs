defmodule UptimeChecker.Repo.Migrations.CreateOrganizationUser do
  use Ecto.Migration

  def change do
    create table(:organization_user) do
      add :status, :integer, default: 1

      add :role_id, references(:role)
      add :user_id, references(:user, on_delete: :delete_all)
      add :organization_id, references(:organization, on_delete: :delete_all)

      timestamps()
    end

    create unique_index(:organization_user, [:user_id, :organization_id])

    create index(:organization_user, [:role_id])
    create index(:organization_user, [:user_id])
    create index(:organization_user, [:organization_id])

    # Partial index
    create unique_index(:organization_user, [:role_id, :user_id],
             name: :uq_superadmin_on_org_user,
             where: "role_id = 1"
           )
  end
end
